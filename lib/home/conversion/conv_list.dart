...
import 'package:extruck/home/conversion/add_item.dart';
            ...
            return SingleChildScrollView(
              child: Column(
                children: [
                  GestureDetector(
                    onTap: () async {
                      var tmp = await db.searchConversionCart(
                        UserData.id,
                      ...
                      setState(() {
                        ...
                        CartData.convUom      = _itemlist[index]['conv_uom'];
                        CartData.convQty      = _itemlist[index]['conv_qty'];
                        // if (_itemlist[index]['status'] == '0') {
                      ...
                      if (_tmpitm.isNotEmpty) {
                        ...
                        final action = await Dialogs.openDialog(context, 'Confirmation', msg, false, 'No', 'Yes');
                        if (action == DialogAction.yes) {
                          showDialog(context: context, builder: (context) => const AddItem()).then((value) {
                            setState(() {
                              refreshList();
                              // loadProducts();
                            });
                        ...
                        showDialog(
                          ...
                          context: context,
                          builder: (context) => const AddItem()).then((value) {
                          setState(() {
                            refreshList();
                            // print('REFRESH!');
                          });
                            ...
                            child: Column(
                              children: [
                                ...
                                Text('${_itemlist[index]['item_qty']} item(s) available',
                                  style: TextStyle(fontSize: 9, fontStyle: FontStyle.italic, fontWeight: FontWeight.w500,
                                    color: lowStock ? Colors.red[800] : Colors.black,
                                  ),
                                ),
                              ],
                            ),
              ...
              Container(
                ...
                child: Align(
                  ...
                  child: Text(formatCurrencyAmt.format(double.parse(Provider.of<CartTotalCounter>(context).totalAmt.toString())),
                    style: const TextStyle(color: Colors.green, fontSize: 12, fontWeight: FontWeight.w500, fontStyle: FontStyle.italic),
                  ),
                ),
              ),
            ],
